...
source "${SCRIPT_DIR}/claude-poll-commands.sh"
source "${SCRIPT_DIR}/claude-write-response.sh"
...
This may take a moment depending on complexity."
    ...
    # CRITICAL: Wrap in error handling to prevent crashes
    {
        ...
        if [[ "$command" == build-feature* ]]; then
            ...
            if [ -f "${SCRIPT_DIR}/build-feature.sh" ]; then
                # Run in subshell to isolate any exit commands
                (
                    "${SCRIPT_DIR}/build-feature.sh" "$feature_name" "$description" 2>&1
                    exit $?
                )
                exit_code=$?
