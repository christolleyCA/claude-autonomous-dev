#!/usr/bin/env python3
"""
IRS Form 990-N (ePostcard) Loader - Small Nonprofit Data

Downloads and processes IRS Form 990-N filings for small nonprofits (<$50K revenue).
Filters for organizations with websites and recent filings (2020+).

Perfect for GrantPilot's target customer base!
"""
...
class Form990NLoader:
    ...
    def download_and_process(self):
        ...
        try:
            ...
            with zipfile.ZipFile(BytesIO(response.content)) as zf:
                ...
                print(f"   ⚡ Filters: Website required + Tax Year 2020+")
                print()
                ...
                with zf.open(txt_file) as f:
                    ...
                    for line_num, line in enumerate(text_file, 1):
                        try:
                            nonprofit = self.parse_line(line.strip())
                            ...
                            if line_num % 50000 == 0:
                                print(f"      {line_num:,} lines processed... ({self.total_with_websites:,} with websites, {self.total_skipped:,} skipped)")
    ...
    def parse_line(self, line):
        ...
        try:
            ...
            # Filter 2: Must have website
            if not website or website.upper() in ['', 'N/A', 'NA', 'NONE', 'NULL']:
                return None

            # Filter 3: Recent filings only (2020+)
    ...
    def run(self):
        ...
        try:
            ...
            print("=" * 80)
            print(f"\n📊 Final Statistics:")
            print(f"   Organizations with websites: {self.total_with_websites:,}")
            print(f"   Skipped (no website or pre-2020): {self.total_skipped:,}")
            print(f"   CSV batches created: {self.batch_number}")
            print(f"   Output directory: {self.output_dir}")
            print(f"\n💾 All data uploaded to Supabase!")
            print(f"✅ Small nonprofits (<$50K) ready for CRM targeting!\n")
